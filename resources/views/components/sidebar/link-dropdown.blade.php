...
<button type="button"
    class="flex items-center w-full p-2 text-base text-yellow-400 transition duration-75 rounded-lg group hover:bg-gray-100  dark:hover:bg-gray-700"
...
<ul id="{{ $id }}" class="{{ $active ? 'block' : 'hidden' }} py-2 space-y-2">
    @foreach ($items as $item)
        <li>
            ...
            <a href="{{ $item['url'] }}"
                class=" {{ $item['active'] ? 'text-primary-500' : 'text-yellow-400 ' }} flex items-center w-full p-2  transition duration-75 rounded-lg pl-11 group ">
                {{ $item['label'] }}
            </a>
            @if ($item['badge'])
            <span
